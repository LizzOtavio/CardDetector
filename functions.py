    ...
    for i in range(0, len(contours)):
        ...
        if area > 5000:
            ...
            try:
                ...
                if dist1 > dist2:
                    pts_src = np.array([[x1, y1], [x2, y2], [x3, y3], [x4, y4]]).reshape((4,2))
                else:
                    pts_src = np.array([[x2, y2], [x3, y3], [x4, y4], [x1, y1]]).reshape((4,2))
                h, status = cv2.findHomography(pts_src, pts_dst)
                img = cv2.warpPerspective(input_img, h, (def_width, def_height))
                img, name, suit = match_templates_to_card(img, all_templates)
                #print('Result: {} de {}'.format(name, suit))
                ptx = min([x1, x2, x3, x4])
                pty = max([y1, y2, y3, y4]) 
                cv2.putText(img,'{} de {}'.format(name, suit),(0, 270,),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,0,0),1)
                cv2.putText(input_img,'{} de {}'.format(name, suit),(ptx, pty),cv2.FONT_HERSHEY_SIMPLEX,0.6,(0,0,0),1)
                cards.append(img)
            except:
                pass
